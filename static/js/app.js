...
async function openCartelaPicker() {
    ...
    if (allCartelas.length === 0) {
        cartelaPage = 1;
        await loadCartelasBatch();
    }
    renderCartelaPicker();
    openModal('modal-cartelas');
}

async function loadCartelasBatch() {
    try {
        // Load all 500 cartelas at once (pre-generated)
        const res = await fetch(`${API_BASE}/api/cartelas/preview`, { headers: getAuthHeaders() });
        const data = await res.json();
        if (cartelaPage === 1) allCartelas = [];
        allCartelas = data.cartelas || [];
        return data;
    } catch (e) {
